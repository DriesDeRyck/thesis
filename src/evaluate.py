# Based on the evaluate.py file from https://github.com/knightlab-analyses/multiomic-cooccurrences
...
def _edge_roc_curve(ranks, edges, k_max=10, axis=1):
    ...
    #                             index=ranks.index)
    # NOTE: sorted ranks = for every microbe, sort the columns such that the first column contains the metabolite with
    ...
    for k in idx:
        ...
        # NOTE: get the k metabolites with the highest coefficient for each microbe
        pos_edges = sranks.iloc[:, -k:].stack().reset_index()[['level_0', 0]]
        neg_edges = sranks.iloc[:, :k].stack().reset_index()[['level_0', 0]]

        for c in ['R', 'C', 'I']:
            # NOTE: changed this such that for 'release' relationships, we look at metabolites with highest coefficient,
            # but for 'consumed' or 'inhibited' we look at metabolites with the lowest coefficient
            if c == 'R':
                res_edges = pos_edges.rename(
                    columns={'level_0': 'microbe', 0: 'metabolite'})
            else:
                res_edges = neg_edges.rename(
                    columns={'level_0': 'microbe', 0: 'metabolite'}
                )

            # NOTE: exp_pos_edges = expected positive edges --> what we expect as ground truth (so for 'R', there are
            # 3 'release' relationships with 100 microbes and 100 metabolites in each relationship, so 30000 in total
            exp_pos_edges = edges.loc[edges.direction == c]
            exp_pos_edges = set(
                map(tuple, exp_pos_edges[['microbe', 'metabolite']].values)
            )
            # NOTE: resulting positive edges --> what we see in the results
            res_pos_edges = set(
                map(tuple, res_edges[['microbe', 'metabolite']].values)
            )
